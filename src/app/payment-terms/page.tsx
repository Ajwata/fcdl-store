...
export default async function PaymentTermsPage() {
  ...
  const paymentTerms = cms.documents.paymentTermsPage;
        ...
        <section className="section-block mx-auto max-w-6xl px-6 py-16 lg:px-10 lg:py-20">
          <div className="rounded-[28px] border border-[var(--blue-100)] bg-[linear-gradient(135deg,#f6fbff_0%,#edf5ff_45%,#f0f9f2_100%)] p-6 shadow-[0_18px_44px_rgba(8,26,51,0.08)] sm:p-8">
            <div className="grid gap-6 lg:grid-cols-[1fr_280px] lg:items-end">
              <div>
                <p className="text-sm font-bold uppercase tracking-[0.24em] text-[var(--green-700)]">{paymentTerms.badge}</p>
              ...
              <div className="rounded-2xl border border-white/70 bg-white/80 p-4 backdrop-blur">
                <p className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--blue-700)]">{paymentTerms.supportLabel}</p>
                <p className="mt-2 text-sm font-semibold text-[var(--blue-950)]">{paymentTerms.supportEmail}</p>
                <p className="mt-1 text-sm font-semibold text-[var(--blue-950)]">{paymentTerms.supportPhone}</p>
              </div>
            </div>
          </div>

          <div className="mt-8 grid gap-4 md:grid-cols-2">
            {paymentTerms.sections.map((item, index) => (
              <article
                key={item.title}
